...
//
// This is the core contract test for the namespace-filtering fix.
func TestGetMatchingRules_MustFilterByNamespaceForNamespacedWatchRule(t *testing.T) {
	...
	sameNS := store.GetMatchingRules(sameNSObject, "services", configv1alpha1.OperationCreate, "", "v1", false)
	if len(sameNS) != 1 {
		t.Errorf("expected same-namespace match to return 1 rule, got %d", len(sameNS))
	}

	foreignNSObject := &unstructured.Unstructured{}
	foreignNSObject.SetNamespace("gitops-reverser")

	// Different namespace — MUST NOT match.
	foreignNS := store.GetMatchingRules(foreignNSObject, "services", configv1alpha1.OperationCreate, "", "v1", false)
	if len(foreignNS) != 0 {
		t.Fatalf("expected foreign-namespace match to return 0 rules, got %d", len(foreignNS))
	}
}

// TestGetMatchingRules_NamespacedWatchRule_NamespaceContract verifies the steady-state matcher contract.
func TestGetMatchingRules_NamespacedWatchRule_NamespaceContract(t *testing.T) {
	...
	store.AddOrUpdateClusterWatchRule(
		configv1alpha1.ClusterWatchRule{
			...
			Spec: configv1alpha1.ClusterWatchRuleSpec{
				TargetRef: configv1alpha1.NamespacedTargetReference{Name: "cluster-target", Namespace: "ops"},
				Rules: []configv1alpha1.ClusterResourceRule{{
					Operations:  []configv1alpha1.OperationType{configv1alpha1.OperationAll},
					APIGroups:   []string{""},
					APIVersions: []string{"v1"},
