...
func TestNewRedisAuditDebugQueue_RequiresAddress(t *testing.T) {
	_, err := NewRedisAuditDebugQueue(RedisAuditQueueConfig{})
	require.Error(t, err)
}

// The debug stream stores the same overview fields the canonical stream once
// did (plus the source); they are what makes the diagnostic stream greppable.
func TestRedisAuditDebugQueue_EnqueueStoresOverviewFields(t *testing.T) {
	mr := miniredis.RunT(t)
	...
	assert.Equal(t, "audit-cr-123", entry["audit_id"])
