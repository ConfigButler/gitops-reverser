...
func TestRaceConditionIntegration(t *testing.T) {
	...
	t.Run("full_race_condition_workflow", func(t *testing.T) {
		...
		result, err := WriteEvents(ctx, repo.path, events, repo.auth)
	...
	t.Run("error_handling", func(t *testing.T) {
		...
		_, err := WriteEvents(ctx, repo.path, []Event{}, repo.auth)
		...
		_, err = WriteEvents(ctx, repo.path, []Event{invalidEvent}, repo.auth)
		_ = err
	})
}
