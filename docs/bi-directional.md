...
- **Healthy and idle:** it finds out on its own, within `--git-refresh-interval` (10m by default)
  ...
  advertisement, fetches only if the branch actually moved, and republishes `status.placement` and
  (on its next publication deadline, at most a minute later) `status.remote`. If that push left content the writer cannot handle, the same read sets
