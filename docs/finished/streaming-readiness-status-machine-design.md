...
> [per-type-streaming-readiness-plan.md](streaming-readiness-status-machine-design.md),
> which fixes
> [watch-replay-watermark-stream-readiness-investigation.md](streaming-readiness-status-machine-design.md).
>
> **Supersedes** the `Synced` condition, the `materialization` counts, and the
> `Pending / Initializing / Synced / Degraded` phase set in
> [status-design-git-target.md](../spec/status-conditions-guide.md) §3.1–§3.3. Those were
