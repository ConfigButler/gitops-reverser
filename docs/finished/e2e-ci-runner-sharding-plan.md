...
- **Sharding across runners is strictly *safer* than raising `--procs` on one
  ...
  races documented in
  [e2e-full-suite-shared-state-investigation.md](../spec/e2e-serial-registry.md)
...
- **Shared-state flakes** within one cluster (stale `GitTarget`/`WatchRule`
  ...
  [shared-state investigation](../spec/e2e-serial-registry.md)).
...
5. **Serial registry stays authoritative.** [e2e-serial-registry.md](../spec/e2e-serial-registry.md)
...
- [e2e-serial-registry.md](../spec/e2e-serial-registry.md) — the authoritative `Serial` set.
- [e2e-full-suite-shared-state-investigation.md](../spec/e2e-serial-registry.md)
