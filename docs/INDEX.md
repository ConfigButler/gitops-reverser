...
| [`proving-the-metric-surface.md`](design/proving-the-metric-surface.md) | **design, unbuilt.** What e2e should assert about the metric surface, written after a metric value that was declared, switched on, surfaced as a condition reason and counted turned out to have no producer at all. Two rules: a metric assertion must not take its trigger from the metric under test (the check added to catch that bug had the same blind spot, and stayed green if every recording call was deleted), and no enum value may be reachable only through an injected fake. The finding that shrank the plan is that **run identity already exists**: `testNamespaceFor` seeds the namespace, so every instrument carrying `gittarget_namespace` or `provider_namespace` is already unique per run, and only identity-free instruments need anything more. Separates cluster reuse (scope to the run) from counter resets (`increase()`, not `max_over_time`, which preserves a maximum instead of reconstructing increments). Puts `source_cluster` on the GitTarget mapping rather than on nine instruments, since `clusterProviderRef` is immutable; keeps a run id out of the operator, because that is a test concern in a product surface and Prometheus already solves it scrape-side. Records that the earlier cardinality argument against `source_cluster` on `commit_requests_total` was wrong: bounded by cluster and outcome, not by save, so it is a capacity call, and the counter staying fleet-wide gives up tenant attribution on saves |
