...
| [`push-cooldown.md`](design/push-cooldown.md) | **design, open.** Whether the fixed `5s` push cooldown still earns its place now that the commit window exists. At the defaults both are `5s` and the window is a rolling silence timer, so for one author writing one target the cooldown provably never batches anything; it engages only on identity changes, several targets sharing a branch, `commit.window: 0s`, atomic writes and resyncs. Prices what removing it would actually delete (about thirty lines and two fields) against what it would not touch (the retained-write, base-trust and replay machinery, which exists for push failure), and recommends swapping the cooldown for the failure backoff the worker is missing rather than deleting it outright |
