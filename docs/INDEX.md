...
is the order that work is built in: six PRs of which **only the last two are breaking, and neither
is about placement**, the corpus harness that turns the examples into the definition of done, and
the design changes the examples produced — three of which dissolved when the model reversed, because
the fields they amended stopped existing.
