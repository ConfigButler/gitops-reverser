...
| [`inbound-push-notification.md`](design/inbound-push-notification.md) | **design, open.** Two changes argued as one. The pre-push fetch in front of every publication cycle can go, because the compare-and-swap push session already reads the remote's ref advertisement, and a cycle that commits nothing still reaches it, which closes the stale-tree no-op hazard. What the push cannot do is tell an *idle* target its folder changed, so `GitPathAccepted=False` survives a Git-side repair until something forces a re-read, and that is the inbound notification's job. Carries the `baseTrusted` invariant and its loss table, the round-trip accounting, a four-commit plan, a per-reason fetch counter with red-first unit and e2e assertions, and the source-line accounting |
