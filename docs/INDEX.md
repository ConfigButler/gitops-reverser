...
| [`inbound-push-notification.md`](design/inbound-push-notification.md) | **design, open.** Nothing can tell a `GitTarget` that its branch moved: five fetch call sites, none on a timer and none reachable from outside the process, so an idle cluster holds a stale folder view (a Git-side repair leaves `GitPathAccepted=False` until something forces a re-read). Records that the chain is already built except its first link, since `reconcile.configbutler.ai/requestedAt` already forces the fetch-and-replay, and that the missing piece is only the adapter from a push payload to that annotation. Prices four options, and is deliberate that this is freshness and efficiency rather than correctness: a publication cycle fetches before it plans, so a write never lands on a stale base |
