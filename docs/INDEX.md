...
| [`inbound-push-notification.md`](design/inbound-push-notification.md) | **design, partly shipped.** Two changes argued as one, and the first of them has landed. The pre-push fetch in front of every publication cycle can go, because the compare-and-swap push session already reads the remote's ref advertisement, and a cycle that commits nothing still reaches it, which closes the stale-tree no-op hazard. What the push cannot do is tell an *idle* target its folder changed, so `GitPathAccepted=False` survives a Git-side repair until something forces a re-read, and that is the inbound notification's job. Carries the `baseTrusted` invariant and its loss table, the round-trip accounting, a per-reason fetch counter with red-first unit and e2e assertions, and the source-line accounting. The conditional fetch, its three-flag state machine and the measured round-trip ledger shipped; the receiver did not, and §8.3 is the wire contract it will serve so that nobody has to invent one. §8.4 adds option E, **taking over the Git host's webhook and forwarding it**, which removes the race between two deliveries (not every race) and, the load-bearing part, means **we never have to work out which orchestrator object syncs a folder**: neither tool derives that mapping either, so an interpreter would be claiming something Argo CD and Flux do not. That collapses the ownership-interpreter prerequisite that [`orchestrator-reconcile-trigger.md`](design/support-boundary/orchestrator-reconcile-trigger.md) §4 puts in front of its drift barrier. It opens by stating which side wins: a captured API write beats Git silently, unchanged, and the cell worth protecting is the object the push changed that we hold no intent about. The hold is an optimization and cannot be a barrier, since windows do not merge across authors and nothing observes an apply converging. Cost recorded: the operator becomes the only thing triggering the reconciler, so that timer has to stay a real backstop |
