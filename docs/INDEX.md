...
[`implementation-plan.md`](layout/implementation-plan.md)
...
[`flux-maintainer-review.md`](layout/flux-maintainer-review.md)
is the second external review, `L1`-`L27` over the layout model, its implementation order, and the six worked
examples. One blocker (`homelab-flux` targets Flux's own bootstrap directory, which makes the
operator a second writer in a folder Flux reconciles), one systemic gap (every `input/` fixture is
a Git document rather than a live object, so the corpus asserts placement and never sanitization),
a naming pass (`layout.kind` reuses TypeMeta's word, `status.layout.kind` puts a bare `kind` in
status, `layout.scope` collides with cluster-versus-namespaced), a push to keep counters out of
`status.layout` and turn `renderRootReason` into a condition reason before PR 3 fixes it, and an
argument that `spec.interval` should keep one name on both objects because that IS the Flux
convention. Ends with the piece the first review did not reach: `allowSourceNamespaceOverride`
names the mechanism rather than the privilege and is a boolean where two policy objects already
answer neighbouring halves, so it becomes `crossNamespaceSources: Deny | Allow |
RequireServiceAccount` beside an `accessFrom` rename, and a long verified analysis of whether to
adopt Flux's `serviceAccountName` impersonation. Verdict: adopt it additively, because it buys
revocation through RBAC (which the current gate structurally cannot do) and composes with
`ClusterProvider.spec.kubeConfig`, but it can never replace `allowedSourceNamespaces` (no RBAC rule
can name a Git folder), it costs watch-stream sharing, and the multiple-service-account axis worth
building is per-CONCERN (a separate identity for sensitive types, mirroring Flux's
`--default-decryption-service-account`) rather than per-namespace.
