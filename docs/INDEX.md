...
The proposal is not built. What it would take, the worked examples that make it concrete, and the
external review of both are separate documents that land beside these as they are written.
...
shipped on `feat/flux-status-contract`, and `F6`, `F9` and `F10` are still open.
