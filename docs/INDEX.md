...
| [`metrics-observability-plan.md`](design/metrics-observability-plan.md) | the canonical metrics plan, rewritten 2026-09-07 after an audit of the live instrument set. Reads the product as one pipeline — watch events arrive, and are processed into commits — and gives each stage one counter with a bounded `outcome`, so the flow and its exceptions are drawable. Finds six exact duplicates, three names that describe something the code does not do, and three silent drops that are log lines with no counter (a full write queue, a failed push, a failed route). Nets **39 instruments against today's 43** while adding the whole ingest stage and push health. Also owns the status half: `status.streams`, `status.retention` and the never-written `status.lastPushTime` come off the CRDs, which corrects two worked examples in [`spec/status-conditions-guide.md`](spec/status-conditions-guide.md). Open: all five phases |
