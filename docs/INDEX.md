...
| [`watch-manager-ownership.md`](design/watch-manager-ownership.md) | **decided, not built, with a staged implementation order.** A rule edit is applied inline by the controller worker that observed it, and it re-plans EVERY GitTarget rather than the one the rule names: 1256 plan reconciles across 28 targets in one e2e run, peaking at 78 in a second, behind two network calls, on a shared worker pool. The watch manager has no owner, so eleven mutexes stand in for one. Proposes the opposite shape: controllers post a trigger naming a GitTarget and return, one loop owns the mutable plan state and paces itself, and repeated triggers for one target collapse into a single pass. Half of it is already in the tree, since `catalogRefreshCh` is exactly this for the API-surface trigger and nothing else uses it. The debounce is framed around how the config is actually edited: a GitTarget and its rules are one piece of configuration applied together, so a per-target ROLLING SILENCE window of 2s (max wait ~10s) turns a five-object `kubectl apply` into one pass, the same mechanism `DefaultCommitWindow` already uses one layer down on the write path. That reverses an earlier revision's "never debounce the first declaration": declaring a GitTarget the instant it lands means declaring it with no rules yet, which manufactures a transient EMPTY plan on every cold start, and an empty plan is what vacuously cleared a write divergence in the fidelity gate. The decision it forces: a WatchRule reconcile can no longer read back the summary it caused, which the existing 10s settle requeue already covers. States the contract as "one settled configuration adjustment, not one function invocation": the window is a heuristic and never a correctness boundary (Kubernetes has no apply-complete event), a per-target DIRTY SEQUENCE means a change arriving mid-pass is never lost, and the pass reads a coherent rule-store snapshot rather than the rule that triggered it. Carries the deletion inventory, because the point is that the system gets smaller: four trigger mechanisms collapse to one (`signalCatalogRefresh` and `catalogRefreshCh` go), `refreshRunningTargetWatches` and its running-set filter go (that filter is why a target whose first declare never completed is never picked up again), and roughly half the mutexes go for stated reasons, with `RenderFidelityGate.mu` and the two event-channel locks kept and justified. Ends in four steps that can be picked up as written: bound the local-cluster discovery call (small, independent, a real defect today), then the owner loop with the debounce, dirty sequence, per-target deadline and 2s/5s/10s/30s/1m backoff, then move the data-plane reports onto the owner channel so the shared-state locks go, then scope catalog invalidation to the affected targets. Open: where persistent per-target failure surfaces as a condition, whether the window ever needs to be configurable, and whether the type-to-target index in step 3 earns its own staleness |
