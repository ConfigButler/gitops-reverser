...
| [`attribution-metrics-proposal.md`](design/attribution-metrics-proposal.md) | a consolidated attribution metric surface, proposed now because this release has already broken the `result` label (`exact_deletecollection_item` is gone, `name` is new), so finishing the job costs the same single migration that deferring it would pay twice. Splits `result` into `tier` and `actor_kind`, which is how `commits_total` already models the same distinction, and splits `weak` into `latest` and `resource_version` so the label is exactly the tier ladder. Adds the ten things nothing can currently see, starting with a fact discarded because it could be filed under no key (the population the window race lived in, where no counter moved), the publish-side tier distribution, and the head-of-line delay a slow resolve imposes on the events queued behind it. Re-reading the fact-stream record found seven more, including a follower that fails and retries with only a log line to show for it, and the two questions that record leaves open (facts per entry, streams followed) that cannot be settled without measuring them. Open: whether to take the label break in this release |
