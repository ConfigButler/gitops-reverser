...
| [`commitrequest-save-wait-options.md`](design/commitrequest-save-wait-options.md) | **design, decided** — a five-phase plan, reported from a live cluster: a `CommitRequest` can expire before the write it exists to publish arrives, because the close delay defaulted to `0` while the write is still held head-of-line waiting for an audit fact, so the save resolves `Ready=True`/`NoWindowInGrace` and the edit commits seconds later under the target's `liveTemplate`. The decision is to repair the default (`spec.closeDelay`, now a duration string defaulting to `"2s"`, **shipped as phase 0**) and then let a request **name the write it is waiting for**, reusing the `(uid, resourceVersion)` identity the attribution join already keys on, including its settled `ExactCapable` rule. Records why the fact index cannot BE the wait (it sits upstream of the watch event, so a fact-keyed wait fires before the window opens) and what to take from it anyway — the register-then-check waiter discipline, and a best-effort retrospective oracle bounded by a TUNABLE TTL. Twelve situations, and the page is deliberate about the two it cannot fix cheaply: row 11 (a saturated queue is rejected in the WATCH path and never reaches the worker, so no worker-side wait can diagnose it) is an accepted, named gap rather than a promise, and row 10 is fixed only by a hook in the fidelity-drop branch that marks a wait terminally UNSATISFIABLE — satisfying before that gate would arm a finalize for a write that never commits. Carries a **preconditions chapter** for saves on today's build (audit policy first, then delay sizing, fidelity warm-up, save-button debounce, and assert on the commit rather than on `Ready`). `CommitterActivity` is deferred, not recommended: by its own arrival-not-existence rule it cannot satisfy the already-arrived case. Open: situation 12 (one actor, two saves, one window — blocks phase 2), whether the fact-index lookup earns its coupling, and how an unsatisfied wait is graded |
...
| [`metrics-observability-plan.md`](design/metrics-observability-plan.md) | **built.** the canonical metrics document, rewritten 2026-09 and shipped in the same wave. Reads the product as two ingestion paths — object state through watch, authorship through audit — joining at the resolver, and gives each stage one counter with a bounded `outcome` so the flow and its loss paths are drawable. Records what three review rounds corrected, including two reversals worth not re-litigating: the fact-loss counters must NOT be merged (an eviction counts a fact, a trim gap an occurrence, a decode error a whole batch), and a cumulative counter cannot replace a current count, which is why `status.streams` and `status.retainedDocuments` stayed. The instrument inventory lives in [`interpreting-metrics.md`](interpreting-metrics.md), not here. Open: the Grafana JSON and alert rules |
| [`reconcile-triggering.md`](design/reconcile-triggering.md) | which controllers still fail to wake up |
| [`push-notification-and-reconcile-trigger.md`](design/push-notification-and-reconcile-trigger.md) | **part shipped, the rest is a menu.** Merges the old `inbound-push-notification.md` and `support-boundary/orchestrator-reconcile-trigger.md`, which argued two halves of one subject: the messages Reverser and the reconciler do not send each other. Part 1 is built and measured: the pre-push fetch is gone because the compare-and-swap push already reads the advertisement and a cycle that commits nothing still reaches it, carried by a three-flag invariant (`baseTrusted`/`worktreeDirty`/`replayRequired`, each bracketing a different span), a measured round-trip ledger, and a per-reason fetch counter. Part 2 is the two gaps: nothing tells an idle target its branch moved, and a refused write produces no commit so nothing reverts it. It also states **which side wins** (a captured API write beats Git silently; the cell worth protecting is the object a push changed that we hold no intent about) and why wiring a webhook to the `requestedAt` annotation is a correctness bug rather than a shortcut. Part 3 is six options with consequences, **doing nothing first**: a maximum age on `baseTrusted`, telling the reconciler after we push (which closes the API-first loop with **no Git-host webhook at all**), receiving a notification, taking the webhook over and forwarding it (so **we never have to work out which orchestrator object syncs a folder** — neither Argo CD nor Flux derives that mapping either), holding the push as an optimization that cannot be a barrier, triggering a reconcile to revert a refused edit, and **pushing an empty commit**, which closes that same gap for nothing: a new SHA defeats Argo CD's `selfHeal: false` skip legitimately (the skip is gated on having already synced *this revision*) and gives Flux a new artifact revision, so neither needs us to name an object, patch one, or hold an Argo sync authority. Its costs are a branch-wide blast radius and what it does under `prune: true`. Option 8 hands the action to the installer instead, and records that **executing a script mounted from a ConfigMap is declined**: it is arbitrary code execution as a process holding Git credentials, SSH and age keys and remote kubeconfigs, and Argo CD removed exactly that pattern in 2.8 in favour of a sidecar. The seam is a structured Event carrying the refused object and reason, which the existing `EventRecorder` makes nearly free. Options 7 and 8 together are why option 6, and the ownership interpreters it needs, may never be built |
| [`push-cooldown.md`](design/push-cooldown.md) | **design, open.** Whether the fixed `5s` push cooldown still earns its place now that the commit window exists. For ONE author writing one target it is provably inert at the defaults, because the window is a rolling silence timer of the same length. But there is only one open window per worker, so a second identity finalizes it on arrival: under alternating authors the silence timer never fires and the cooldown is the only thing batching, which makes a multi-user attributed cluster the normal case rather than the exception. Prices what removal would delete (about thirty lines and two fields) against what it would not touch (the retained-write, base-trust and replay machinery, which exists for push failure), and concludes that the unambiguous gap is the missing failure backoff rather than the cooldown itself |
...
`--author-attribution-transport=memory` runs attribution with no Redis on a single replica (the
watch resume cursors still live in Redis, so this frees attribution from it rather than the install).
