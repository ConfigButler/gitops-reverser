...
> [`../api-wave.md`](../../design/gittarget-api-wave.md). The current supported Kustomize
> boundary remains in
...
so every document carries its own. `false` is honest only when something guarantees the namespace,
which the post-scan pass re-checks on every scan, and which
[empty-repo-bootstrap](empty-repo-bootstrap/README.md) makes provable by having the operator write
the root that supplies it.
