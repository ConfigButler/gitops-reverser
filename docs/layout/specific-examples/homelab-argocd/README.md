...
[`config/gittarget.yaml`](config/gittarget.yaml) sets `serializeNamespace: false` and constrains
the source to one exact namespace.
...
correctness rule, the pass that runs on every scan re-checks the supplier and reports
`Validated=False` naming the field and what the folder actually contains, rather than letting the
relocation happen silently.
