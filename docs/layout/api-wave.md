...
> [`model.md`](model.md) has since reversed: the path template stays and gains two additive fields,
...
[`open-asks-priority.md`](../design/open-asks-priority.md) — the removal-wait decision, #15's
...
`serializeNamespace`, `kustomizeRoot`, `commitWindow` and `commit.message` — five more members on a
...
- **`serializeNamespace` and `kustomizeRoot` nest under `spec.placement`**, where
  [`model.md`](model.md) had them at the top level beside it. They are placement concerns by their
  own argument — one decides what the produced document contains, the other what governs it — and
  `spec.placement` is an existing optional struct, so nesting them is **still purely additive**. It
  costs no bump, it is free only before they exist, and it means the placement axis is one member of
...
`Create` writes a `kustomization.yaml` the user did not author. That raises the stakes on the
...
[`open-asks-priority.md`](../design/open-asks-priority.md) left open whether a fall-back to canonical
...
[`implementation-plan.md`](implementation-plan.md)'s PR 3.
