...
objects are generated by a controller or an installer of your own, three of them change shape:

- **The inventory is a code search, not a cluster query.** `kubectl ... | jq` tells you what is
  deployed today, which you already know: whatever your generator emits. Grep the generator for the
  old spellings instead, and treat its templates as the artifact to migrate.
- **The apply happens on the controller's schedule, not yours.** There is no single-sync moment you
  control, so the fix is a code change plus a redeploy, and the objects converge whenever your
  reconciler next writes them. Plan for a window where old and new spellings are both in flight.
- **The verification step is a test, not a `kubectl get`.** A controller that applies with
  `fieldValidation: Ignore` is the quiet client described above: it is accepted and pruned, and the
  object stays `Ready=True`. Assert the **absence** of the old spelling in your own test suite,
  because nothing in the object's status distinguishes migrated from silently defaulted.

That last point is the one worth spending a test on. It is the difference between finding out at
upgrade time and finding out when a value you thought you had set turns out never to have applied.
...
**If you granted the reverser broad RBAC *because* the destination policy was doing the fencing,
re-read your ClusterRole: it is now the fence.** The tenancy boundary did not disappear in this
release, it relocated. It used to be declared by the destination, visible on the `GitTarget`, and
reviewable in a pull request alongside the rule it constrained. It is now a property of the
credential, expressed in RBAC and reviewed wherever you review RBAC. A narrow, hand-written
ClusterRole that was defence-in-depth before this release is load-bearing after it, and the
packaged `*/*` role fences nothing at all. This is worth writing down next to the role itself, so
that nobody widens it later on the assumption that something downstream is still checking.
...
3. the built-in canonical path, `{namespace}/{groupPath}/{resource}/{name}{sensitiveSuffix}`:
   `{namespace}` renders the literal `_cluster` for a cluster-scoped resource, a core resource
...
**If you import this repo as a Go module** — `pkg/manifestanalyzer` is the usual reason — it affects
you before any other step on this page, because it stops the build rather than the migration:

```console
$ go get github.com/ConfigButler/gitops-reverser@v0.43.0
go: github.com/ConfigButler/gitops-reverser@v0.43.0 requires go >= 1.27.0
    (running go 1.26.5; GOTOOLCHAIN=local)
```

The official `golang` images set `GOTOOLCHAIN=local`, so the container's Go is the only Go there is
and this is a hard stop, not a download prompt. It also cascades into linting: `golangci-lint`
refuses a target newer than the Go it was built with, and reports it as a config-load error rather
than as a finding, which reads like a broken config file rather than a version floor:

```console
Error: can't load config: the Go language version (go1.26) used to
build golangci-lint is lower than the targeted Go version (1.27.0)
```

### Migration

Move the pinned Go toolchain **and** the pinned `golangci-lint` in your build image together, before
starting any other migration on this page. Upgrading one without the other trades the first error
for the second.

### Going forward

A `go` directive move is treated as consumer-breaking for module users. It gets an entry here and a
line in the release notes even when it ships in a patch release, so it is never again something you
discover from a failing build.
