[ ] **QUEUE COLLISION ISSUE**: Currently, each GitProvider gets its own queue (keyed by namespace/name). If two different GitProviders point to the same Git repository URL, they will have separate queues and may cause write collisions when pushing simultaneously. Consider:
    - Option 1: Use repository URL as queue key (requires fetching GitProvider on dispatch)
    - Option 2: Add validation webhook to prevent multiple GitProviders with same repo URL
    - Option 3: Document as known limitation and recommend using single GitProvider per repo
...
* If the AccessPolicy is adjusted on the GitProvider, are the existing watchrules also re-evaluated (if they can send in events).
...
* Improve README.m
  * Better explaination of configuration of this tool: one GitProvider per repo, security considerations (namespace or non namespace etc), storeRawConfigmaps (default false).
