...
- **Investigation tie-in (the risk is broader than divert).** Two failure modes for audit-scan
  ...
  1. **Divert.** Since the late lane was removed
     ([audit-diagnostic-streams-plan.md](audit-diagnostic-streams-plan.md)),
...
1. **B-1 — attribution store at ingestion (written early).** Add the store + the webhook hook that
   writes a `{ns,name,uid}→{author,auditID,ts}` record on a `commitrequests` create. Write it **early
   in the handler — before the `canonicalMu` serialization and the ordering/divert decision** — so it
   is divert-immune, scan-miss-immune, and adds no in-lock latency (§5.1). Unit-test the write + the
   divert case (a create that would have been diverted still records its author).
2. **B-2 — finalizer reads the store.** Swap `LookupCommitRequestAuthor` to read the store by key;
...
3. **B-3 — drop the demand.** Remove `commitrequests` from `AlwaysAllow`; confirm the type is no
   longer mirrored (a dropped per-type stream) and attribution still works. Delete
   `commitrequest_author.go`'s stream scan.
...
- **Open:** HA — the store is shared (Redis), so it is naturally multi-pod; confirm the ingestion
  hook writes it on whichever pod handles the webhook (any pod), which it does (the handler runs on
  every pod).
