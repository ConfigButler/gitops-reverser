...
2. **Preservation cascade (test infrastructure).** When a spec fails, every subsequent spec's `DeferCleanup` is skipped to keep state available for investigation. The skipping is global, not per-spec: a single failure leaves a trail of fully alive WatchRules and GitTargets in shared namespaces, which then cause unrelated specs (notably the signing batch spec) to fail for reasons that have nothing to do with their own logic. This makes the suite's failure output misleading — a single root failure produces a wave of secondary failures that look independent. Options: scope preservation to the failing spec only; run preserved-mode specs in throwaway namespaces so the leaked WatchRules cannot match later test fixtures; or skip subsequent specs entirely once preservation kicks in.
...
4. Scope the "preserve resources for investigation" behavior to the failing spec only — or skip dependent specs after preservation kicks in — so a single failure does not produce a wave of secondary failures masking the actual root cause.
5. Replace the 30s window in the signing batch spec with a wait against a controller log line or metric that confirms the snapshot-batch loop ran, so the spec asserts on causality rather than a wall-clock budget.
