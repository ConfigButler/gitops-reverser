...
`deletecollection` rework removed `exact_deletecollection_item` and added `collection_uid`,
`collection_scope`, and `name` — so finishing the job costs the same single migration, and deferring
