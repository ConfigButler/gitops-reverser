...
type CommitMessageSpec struct {
	...
	// Resources exposes Operation, Group, Version, Resource, Namespace, Name, APIVersion,
	// ResourceVersion — the version of the state this commit wrote, which lags the object's
	// current one and is empty for a producer that observed none — and Generation, the desired
	// state's counter, which is 0 for a kind without a spec. Guard both with {{with}}.
