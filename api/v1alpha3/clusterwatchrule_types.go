...
type ClusterResourceRule struct {
	...
	// Scope is REMOVED as a choice: a ClusterWatchRule is cluster-scoped only, so the only
	// accepted value is "Cluster" — which is also the default, making the field omittable.
	//
	// To watch NAMESPACED resources, use a WatchRule in the tenant namespace and set
	// spec.rules[].sourceNamespace ("*" reaches every namespace the GitTarget admits).
	//
	// The field is retained in the schema for one release purely so that re-applying a manifest
	// that still says "Namespaced" FAILS. Deleting it outright would be worse and silent twice
	// over: CRD pruning happens on write, so the value would be dropped without an error and the
	// rule would quietly stop mirroring namespaced objects; and a stored pre-release object would
	// keep its value in etcd with no Go field left to read, leaving the controller nothing to
	// refuse. The narrowed enum rejects it at admission, and the compile path refuses a stored
	// value with a message naming the replacement.
	//
...
// from. It is the cluster-scoped half of the two-object model: scope is carried by the rule KIND,
// so it has no per-rule scope choice and no source-namespace selection at all.
//
// Security model:
//   - ClusterWatchRule is cluster-scoped and requires cluster-admin permissions.
//   - It references a GitTarget via targetRef (namespace required), and the referenced target's
//     namespace must be admitted by that target's ClusterProvider.
//   - Cluster-scoped objects have no namespace, so GitTarget.spec.allowedSourceNamespaces is
//     neither consulted nor a bound for them. A ClusterWatchRule is intentionally cluster-global
//     and is limited only by its source credential's Kubernetes RBAC. Isolating cluster-scoped
//     objects between tenants therefore takes separate credentials/ClusterProviders, not a
...
//
// To mirror NAMESPACED resources, use a WatchRule in the tenant namespace and set
// spec.rules[].sourceNamespace — "*" reaches every namespace the GitTarget's
// allowedSourceNamespaces admits, which replaces the removed cluster-wide namespaced scope.
