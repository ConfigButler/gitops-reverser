...
// IceCreamOrder CRD, restart_reconcile mirrors CRDs) through the controller's global CRD-discovery
...
var _ = Describe("Manager CRD Lifecycle", Label("manager"), Serial, Ordered, func() {
	...
	It("should create Git commit when IceCreamOrder is added via WatchRule", func() {
		...
		verifyGitCommit := func(g Gomega) {
			...
			expectedFile := filepath.Join(crdLifecycleRepo.CheckoutDir,
				"e2e/icecream-test",
				iceCreamInstancePath(crdGroupCRDLifecycle, testNs, crdInstanceName))
			fileInfo, statErr := os.Stat(expectedFile)
			g.Expect(statErr).
